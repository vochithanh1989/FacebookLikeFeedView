...
public class SendMessageAdapter extends BaseAdapter {
	...
	public SendMessageAdapter(Context context, ArrayList<Message> messages) {
		super();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		{		
			...
			if(message.isMine())
			{
				holder.message.setBackgroundResource(R.drawable.backgroup_messenger);
				lp.gravity = Gravity.RIGHT;
			}
			//If not mine then it is from sender to show orange background and align to left
			else
			{
				holder.message.setBackgroundResource(R.drawable.backgroup_messenger2);
				lp.gravity = Gravity.LEFT;
			}
